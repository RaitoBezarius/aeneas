    ...
    object (self)
      ...
      method! visit_Let env monadic lv rv next_e =
        ...
        | Switch _ | Loop _ | Let _ ->
            ...
            not_simpl_e
        | _ -> (
            match typed_pattern_to_texpression def.meta lv with
            | None -> not_simpl_e
            | Some lv_v ->
                let lv_v =
                  if monadic then mk_result_return_texpression def.meta lv_v else lv_v
                in
      ...
      method! visit_texpression env e =
        ...
        | Some (fun_id, _tys, args) -> (
            ...
            | Fun (FromLlbc (FunId (FAssumed aid), _lp_id, rg_id)) -> (
                ...
                | BoxNew, _ ->
                    cassert (rg_id = None) def.meta "TODO: error message";
