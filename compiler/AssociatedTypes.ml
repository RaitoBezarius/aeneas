    ...
    List.filter_map
      (fun (k, v) ->
        ...
        | TTraitType (trait_ref, generics, type_name) ->
            cassert_opt_meta (generics = empty_generic_args) meta "TODO: error message";
...
let rec norm_ctx_normalize_ty (ctx : norm_ctx) (ty : ty) : ty =
  ...
  | TTraitType (trait_ref, generics, type_name) -> (
      ...
      cassert (generics = empty_generic_args) meta "Higher order types are not supported yet TODO: error message";
      let ty : ty =
