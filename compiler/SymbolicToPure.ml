...
let rec translate_back_ty (meta : Meta.meta) (type_infos : type_infos)
  ...
  | TTraitType (trait_ref, generics, type_name) ->
      ...
      cassert (
        AssociatedTypes.trait_instance_id_is_local_clause trait_ref.trait_id) meta "TODO: error message";
      if inside_mut then
  ...
  let func =
    if !Config.return_back_funs then
      ...
      let fun_id =
        ...
        | S.Unop _ | S.Binop _ -> craise ctx.fun_decl.meta "Unreachable"
      in
...
let compute_output_ty_from_decomposed (meta : Meta.meta) (dsg : Pure.decomposed_fun_sig) : ty =
  cassert !Config.return_back_funs meta "TODO: error message";
  (* Compute the arrow types for all the backward functions *)
...
let translate_fun_sig_from_decomposed (meta : Meta.meta) (dsg : Pure.decomposed_fun_sig)
  ...
  let inputs, output =
    ...
    if !Config.return_back_funs then (
      cassert (gid = None) meta "TODO: error message";
      let output = compute_output_ty_from_decomposed meta dsg in
        ...
        *)
        ...
        let ctx, ignore_fwd_output, back_funs_map, back_funs =
          if !Config.return_back_funs then (
            ...
            let dsg =
              translate_fun_sig_with_regions_hierarchy_to_decomposed ctx.fun_decl.meta decls_ctx
...
and translate_end_abstraction_fun_call (ectx : C.eval_ctx) (abs : V.abs)
  ...
  *)
  ...
  then (
    ...
    cassert (List.length inputs = List.length fwd_inputs) ctx.fun_decl.meta "The backward function should have exactly the same number of inputs as the forward";
    next_e)
  else
  ...
  *)
  let v =
    ...
    | VaTraitConstValue (trait_ref, generics, const_name) ->
        let type_infos = ctx.type_ctx.type_infos in
        let trait_ref = translate_fwd_trait_ref ctx.fun_decl.meta type_infos trait_ref in (* TODO: check if right meta *)
        let generics = translate_fwd_generic_args ctx.fun_decl.meta type_infos generics in
...
and translate_loop (loop : S.loop) (ctx : bs_ctx) : texpression =
    ...
    if !Config.return_back_funs then
      ...
      let output =
        ...
        | None ->
            ...
            (translate_fun_sig_from_decomposed ctx.fun_decl.meta ctx.sg None).output
        | Some rg_id ->
  ...
  cassert (Option.is_some ctx.bid || fun_end.ty = loop_body.ty) ctx.fun_decl.meta "The return type of a loop body should be the same as the parent function";
...
let translate_fun_decl (ctx : bs_ctx) (body : S.expression option) : fun_decl =
  ...
  let signature = translate_fun_sig_from_decomposed def.meta ctx.sg ctx.bid in
  let regions_hierarchy =
  ...
  let body =
    ...
    | Some body ->
        ...
        let backward_inputs : var list =
          ...
          | None -> []
          | Some back_id ->
              cassert (not !Config.return_back_funs) def.meta "TODO: error message";
              let parents_ids =
