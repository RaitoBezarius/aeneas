 ...
 *)
let bs_ctx_register_backward_call (meta : Meta.meta) (abs : V.abs) (effect_info : fun_effect_info)
  ...
  let func =
    if !Config.return_back_funs then
      ...
      let fun_id =
        ...
        | S.Fun (fid, _) ->
            let fid = translate_fun_id_or_trait_method_ref meta ctx fid in
            Fun (FromLlbc (fid, None, Some back_id))
        | S.Unop _ | S.Binop _ -> craise meta "Unreachable"
      in
...
let rec translate_expression (metadata : Meta.meta) (e : S.expression) (ctx : bs_ctx) : texpression =
  ...
  | EvalGlobal (gid, sv, e) -> translate_global_eval metadata gid sv e ctx
...
and translate_end_abstraction_fun_call (meta : Meta.meta) (ectx : C.eval_ctx) (abs : V.abs)
  ...
  let fun_id =
    ...
    | Unop _ | Binop _ ->
        ...
        craise meta "Unreachable"
  in
  let effect_info = get_fun_effect_info meta ctx fun_id None (Some rg_id) in
  let generics = ctx_translate_fwd_generic_args meta ctx call.generics in
  ...
  let ctx, func =
    bs_ctx_register_backward_call meta abs effect_info call_id rg_id inherited_inputs
...
and translate_global_eval (meta : Meta.meta) (gid : A.GlobalDeclId.id) (sval : V.symbolic_value)
    (e : S.expression) (ctx : bs_ctx) : texpression =
...
let translate_fun_decl (meta : Meta.meta) (ctx : bs_ctx) (body : S.expression option) : fun_decl =
  ...
  let body =
    ...
    | Some body ->
        let effect_info =
          get_fun_effect_info meta ctx (FunId (FRegular def_id)) None bid
        in
