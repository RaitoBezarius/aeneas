...
let ctx_get_local_function (meta : Meta.meta) (id : A.FunDeclId.id) (lp : LoopId.id option)
  ...
  ctx_get_function meta (FromLlbc (FunId (FRegular id), lp, rg)) ctx
...
let ctx_get_trait_method (id : trait_decl_id) (item_name : string)
    ...
    let meta = (TraitDeclId.Map.find id ctx.trans_trait_decls).meta in
  ctx_get meta (TraitMethodId (id, item_name, rg_id)) ctx
...
*)
let default_fun_suffix (meta : Meta.meta) (num_loops : int) (loop_id : LoopId.id option)
  ...
  *)
  let rg_suff =
    ...
    | Some rg ->
        cassert (num_region_groups > 0 && num_backs > 0) meta "T";
        if num_backs = 1 then
...
let ctx_compute_fun_name (meta : Meta.meta) (ctx : extraction_ctx) (fname : llbc_name)
  ...
  (* Compute the suffix *)
  let suffix = default_fun_suffix meta num_loops loop_id num_rgs rg filter_info in
  (* Concatenate *)
  fname ^ suffix
...
let ctx_add_global_decl_and_body (def : A.global_decl) (ctx : extraction_ctx) :
  ...
  | None ->
      ...
      let ctx = ctx_add def.meta decl (name ^ "_c") ctx in
      let ctx = ctx_add def.meta body (name ^ "_body") ctx in
      ctx
...
let ctx_compute_fun_name (trans_group : pure_fun_translation) (def : fun_decl)
  ...
  ctx_compute_fun_name def.meta ctx def.llbc_name def.num_loops def.loop_id num_rgs
...
let ctx_add_fun_decl (trans_group : pure_fun_translation) (def : fun_decl)
  ...
  let ctx = ctx_add def.meta (FunId (FromLlbc fun_id)) def_name ctx in
  (* Add the name info *)
  {
    ctx with
    fun_name_info =
