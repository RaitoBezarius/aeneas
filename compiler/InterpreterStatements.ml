...
let rec eval_statement (config : config) (st : statement) : st_cm_fun =
 fun cf ctx ->
  ...
  let cc = greedy_expand_symbolic_values st.meta config in
  (* Sanity check *)
...
and eval_global (config : config) (dest : place) (gid : GlobalDeclId.id) :
 ...
 fun cf ctx ->
  ...
  | SymbolicMode ->
      ...
      cassert (ty_no_regions global.ty) global.meta "TODO: Error message";
      let sval = mk_fresh_symbolic_value global.meta global.ty in
      let cc =
