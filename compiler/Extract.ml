...
and extract_App (meta : Meta.meta) (ctx : extraction_ctx) (fmt : F.formatter) (inside : bool)
  ...
  | Qualif qualif -> (
      ...
      | Global global_id -> extract_global meta ctx fmt global_id
      | AdtCons adt_cons_id ->
      ...
      | TraitConst (trait_ref, generics, const_name) ->
          ...
          extract_trait_ref meta ctx fmt TypeDeclId.Set.empty false trait_ref;
...
let extract_fun_decl_gen (ctx : extraction_ctx) (fmt : F.formatter)
  ...
  (* Retrieve the function name *)
  let def_name =
    ctx_get_local_function def.meta def.def_id def.loop_id def.back_id ctx
  in
  (* Add a break before *)
...
let extract_global_decl (ctx : extraction_ctx) (fmt : F.formatter)
  ...
  cassert (body.is_global_decl_body) meta "TODO: error message";
  cassert (Option.is_none body.back_id) meta "TODO: error message";
  cassert (body.signature.inputs = []) meta "TODO: error message";
  cassert (body.signature.generics = empty_generic_params) meta "TODO: error message";
  ...
  | None ->
      ...
      if !backend = HOL4 then
        extract_global_decl_hol4_opaque meta ctx fmt decl_name decl_ty
      else extract_global_decl_body_gen meta ctx fmt kind decl_name decl_ty None
  | Some body ->
      (* There is a body *)
      (* Generate: [let x_body : result u32 = Return 3] *)
      extract_global_decl_body_gen meta ctx fmt SingleNonRec body_name body_ty
        (Some (fun fmt -> extract_texpression meta ctx fmt false body.body));
      F.pp_print_break fmt 0 0;
      (* Generate: [let x_c : u32 = eval_global x_body] *)
      extract_global_decl_body_gen meta ctx fmt SingleNonRec decl_name decl_ty
        (Some
           (fun fmt ->
             let body =
               match !backend with
          ...
          (fun (item_name, fun_id) ->
            ...
            let find (trans_fun : fun_decl) =
              ...
              | None ->
                  ...
                  in
                  log#serror err;
                  if !Config.fail_hard then craise trans_fun.meta err
    ...
    let ty () =
      ...
      (* Note that we do not filter the LLBC generic parameters.
         ...
         - we only generate trait clauses for the clauses we find in the
           pure generics *)
      let ctx, type_params, cg_params, trait_clauses =
        ctx_add_generic_params f.meta f.llbc_name f.signature.llbc_generics generics
          ctx (* TODO: confirm it's the right meta*)
      in
...
let extract_trait_impl_method_items (ctx : extraction_ctx) (fmt : F.formatter)
  ...
  let extract_method (f : fun_and_loops) =
    ...
    let ty () =
      ...
      F.pp_print_space fmt ();
      let fun_name = ctx_get_local_function f.meta f.def_id None f.back_id ctx in
      F.pp_print_string fmt fun_name;
      let all_generics =
...
let extract_unit_test_if_unit_fun (ctx : extraction_ctx) (fmt : F.formatter)
  ...
  cassert (def.back_id = None) def.meta "TODO: error message";
